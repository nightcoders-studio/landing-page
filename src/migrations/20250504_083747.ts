...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`portfolios_populated_authors\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`portfolios_populated_authors_order_idx\` ON \`portfolios_populated_authors\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`portfolios_populated_authors_parent_id_idx\` ON \`portfolios_populated_authors\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`portfolios\` (
  	...
  	\`review_review_image_id\` integer,
  	\`review_review_text\` text,
  	\`review_review_author\` text,
  	\`features\` text,
  	\`use_cases\` text,
  	\`tech_stacks_front_end\` text,
  	\`tech_stacks_back_end\` text,
  	\`tech_stacks_design\` text,
  	\`content\` text,
  	\`meta_title\` text,
  	\`meta_image_id\` integer,
  	\`meta_description\` text,
  	...
  	\`_status\` text DEFAULT 'draft',
  	FOREIGN KEY (\`hero_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`review_review_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`meta_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`portfolios_hero_image_idx\` ON \`portfolios\` (\`hero_image_id\`);`)
  await db.run(sql`CREATE INDEX \`portfolios_review_review_review_image_idx\` ON \`portfolios\` (\`review_review_image_id\`);`)
  await db.run(sql`CREATE INDEX \`portfolios_meta_meta_image_idx\` ON \`portfolios\` (\`meta_image_id\`);`)
  await db.run(sql`CREATE INDEX \`portfolios_slug_idx\` ON \`portfolios\` (\`slug\`);`)
  ...
  await db.run(sql`CREATE INDEX \`portfolios_rels_categories_id_idx\` ON \`portfolios_rels\` (\`categories_id\`);`)
  await db.run(sql`CREATE INDEX \`portfolios_rels_users_id_idx\` ON \`portfolios_rels\` (\`users_id\`);`)
  await db.run(sql`CREATE TABLE \`_portfolios_v_version_populated_authors\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_uuid\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_portfolios_v_version_populated_authors_order_idx\` ON \`_portfolios_v_version_populated_authors\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_portfolios_v_version_populated_authors_parent_id_idx\` ON \`_portfolios_v_version_populated_authors\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_portfolios_v\` (
  	...
  	\`version_hero_image_id\` integer,
  	\`version_about_this_project\` text,
  	\`version_review_review_image_id\` integer,
  	\`version_review_review_text\` text,
  	\`version_review_review_author\` text,
  	\`version_features\` text,
  	\`version_use_cases\` text,
  	\`version_tech_stacks_front_end\` text,
  	\`version_tech_stacks_back_end\` text,
  	\`version_tech_stacks_design\` text,
  	\`version_content\` text,
  	\`version_meta_title\` text,
  	\`version_meta_image_id\` integer,
  	\`version_meta_description\` text,
  	...
  	FOREIGN KEY (\`version_hero_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_review_review_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_meta_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_portfolios_v_parent_idx\` ON \`_portfolios_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_portfolios_v_version_version_hero_image_idx\` ON \`_portfolios_v\` (\`version_hero_image_id\`);`)
  await db.run(sql`CREATE INDEX \`_portfolios_v_version_review_version_review_review_image_idx\` ON \`_portfolios_v\` (\`version_review_review_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_portfolios_v_rels_categories_id_idx\` ON \`_portfolios_v_rels\` (\`categories_id\`);`)
  await db.run(sql`CREATE INDEX \`_portfolios_v_rels_users_id_idx\` ON \`_portfolios_v_rels\` (\`users_id\`);`)
  await db.run(sql`CREATE TABLE \`stores_populated_authors\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`stores_populated_authors_order_idx\` ON \`stores_populated_authors\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`stores_populated_authors_parent_id_idx\` ON \`stores_populated_authors\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`stores_hero_image_idx\` ON \`stores\` (\`hero_image_id\`);`)
  await db.run(sql`CREATE INDEX \`stores_review_review_review_image_idx\` ON \`stores\` (\`review_review_image_id\`);`)
  await db.run(sql`CREATE INDEX \`stores_meta_meta_image_idx\` ON \`stores\` (\`meta_image_id\`);`)
  await db.run(sql`CREATE INDEX \`stores_slug_idx\` ON \`stores\` (\`slug\`);`)
  ...
  await db.run(sql`CREATE INDEX \`stores_rels_users_id_idx\` ON \`stores_rels\` (\`users_id\`);`)
  await db.run(sql`CREATE TABLE \`_stores_v_version_populated_authors\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_uuid\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_stores_v_version_populated_authors_order_idx\` ON \`_stores_v_version_populated_authors\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_stores_v_version_populated_authors_parent_id_idx\` ON \`_stores_v_version_populated_authors\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`_stores_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_title\` text,
  	\`version_price_with_currency\` text,
  	\`version_period\` text,
  	\`version_hero_image_id\` integer,
  	\`version_about_this_project\` text,
  	\`version_features\` text,
  ...
  await db.run(sql`CREATE INDEX \`_stores_v_version_version_hero_image_idx\` ON \`_stores_v\` (\`version_hero_image_id\`);`)
  await db.run(sql`CREATE INDEX \`_stores_v_version_review_version_review_review_image_idx\` ON \`_stores_v\` (\`version_review_review_image_id\`);`)
  await db.run(sql`CREATE INDEX \`_stores_v_version_meta_version_meta_image_idx\` ON \`_stores_v\` (\`version_meta_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_stores_v_rels_users_id_idx\` ON \`_stores_v_rels\` (\`users_id\`);`)
